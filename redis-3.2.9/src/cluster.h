...
typedef struct clusterNode {
     ...
     * 集群两个节点之间相互发送消息
     * 如果节点A发送MEET消息给节点B
     * 那么节点A会创建一个clusterLink结构的连接,fd设置为连接后的套节字,node设置为节点B,最后将该clusterLink结构保存到节点B的link中
     * ******/
...
typedef struct {
    ...
    uint32_t notused2;
} clusterMsgDataGossip;//集群gossip消息结构
